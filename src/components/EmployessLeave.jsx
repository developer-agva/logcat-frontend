...
import { deleteLeaveRequestAction, getCompoffDataAction, getEmployeLeaveStatusAction, getUserDataAction, postrevertLeaveRequest } from "../store/action/userDataAction";
...
const EmployessLeave = () => {
    ...
    const [openUndoModel, setOpenUndoModel] = useState(false); // Default is 'All'
                ...
                <table className="w-full text-left text-sm text-gray-500">
                    ...
                    <tbody>
                        {dataa1?.data?.map((item, index1) => {
                            return (
                                ...
                                >
                                    ...
                                    {/* dispatch(deleteLeaveCompoffAction({ id: item?._id }))  */}
                                    <td>
                                        {item.status === 'Pending' ?
                                            <button className="px-2 py-2 bg-red-600 text-black text-white rounded" onClick={() => { handelOpenModel }}>Delete</button>
                                            : '---'
                                        }
                                    </td>
                                </tr>
                        ...
                        {filteredLeaveData?.map((leave, index) => (
                            ...
                            >
                                <td className="px-4 py-4 text-black text-start flex items-center gap-2">
                                ...
                                <td className="px-4 py-4 text-black text-start">0{leave.totalDays}</td>
                                ...
                                <td>
                                    {leave.status === 'Pending' ?
                                        <button className="px-2 py-2 bg-red-600 text-black text-white rounded" onClick={() => { dispatch(deleteLeaveRequestAction({ id: leave?._id })) }}>Delete</button>
                                        :
                                        leave.status === 'Approved' ?
                                            <button className="px-2 py-2 bg-blue-600 text-black text-white rounded" onClick={() => {
                                                setOpenUndoModel(true)
                                                setUserId(leave?._id)
                                            }
                                            }>UNDO</button>
