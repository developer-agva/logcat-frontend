...
export const addExpencesDataAction = ({ amount, description, time ,isAvlBill }) => async (dispatch) => {
  try {
    dispatch({
...
export const getAllExpenceDataAction = (startDate,endDate) => async (dispatch) => {
  try {
    dispatch({
      type: GET_ALL_EXPENCE_DATA_REQUEST
    });
    const token = cookies.get('ddAdminToken');
    const startDatee=startDate?startDate.split("-").reverse().join("-"):'';
...
export const postDemoAddDataAction = ({ deviceId,serialNo, contactNo, hospitalName, demoDuration, priority, description }) => async (dispatch) => {
  try {
    dispatch({
    ...
    let { data } = await axios.post(
      ...
      { deviceId,serialNo, contactNo, hospitalName, demoDuration, priority, description },
      config
    );
    dispatch({
...
export const postSalesAddDataAction = ({deviceId, serialNo, contactNo, amount, hospitalName }) => async (dispatch) => {
  try {
    dispatch({
