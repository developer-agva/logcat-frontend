...
export const addExperienceAction = ({ userId, associationName, workAddress, startDate, workEmail, endDate, designation, department, workType, workPhoneNo }) => async (dispatch) => {
  try {
    dispatch({
    ...
    const response = await axios.post(
      ...
      { userId, associationName, workAddress, startDate, workEmail, endDate, designation, department, workType, workPhoneNo },
      config
    );
    dispatch({
    ...
    if(response && response.data && response.data.data && response.data.data.statusCode==200){
      toast.success(response && response.data && response.data.data && response.data.data.message)
    }
