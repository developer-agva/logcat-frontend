...
export const getDeveloperOptionData = (did) => async (dispatch) => {
  try {
    dispatch({
    ...
    let response;
    response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/api/logger/logs/get-debug-events/009`,
      config
    );
    dispatch({
...
export const getDeviceEventsById =
  ...
  async (dispatch) => {
    try {
      ...
      response = await axios.get(
        projectCode?`${process.env.REACT_APP_BASE_URL}/api/logger/logs/v2/deviceEvents/${DeviceId12}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`: `${process.env.REACT_APP_BASE_URL}/api/logger/logs/deviceEvents/${DeviceId12}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`,
        config
      );
      dispatch({
...
export const getDeviceAlarmsById =
  ...
  async (dispatch) => {
    try {
      ...
      response = await axios.get(
        projectCode?`${process.env.REACT_APP_BASE_URL}/api/logger/logs/v2/deviceAlerts/${DeviceId121}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`: `${process.env.REACT_APP_BASE_URL}/api/logger/logs/deviceAlerts/${DeviceId121}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`,
        config
      );
      dispatch({
...
export const getDeviceTrendsById = () => async (dispatch) => {
  try {
    ...
    response = await axios.get(
      projectCode? `${process.env.REACT_APP_BASE_URL}/api/logger/logs/v2/deviceTrends/${DeviceId12}`:`${process.env.REACT_APP_BASE_URL}/api/logger/logs/deviceTrends/${DeviceId12}`,
...
export const getRequestUserDataAction = (deviceId) => async (dispatch) => {
  try {
    ...
    if (data && data.statusCode === 200) {
      ...
      localStorage.setItem("reqStatus", "Pending");
    }
  } catch (error) {
    if (
      error &&
