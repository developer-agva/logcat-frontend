...
export const postLockToDeviceIdAction =
  ({ DeviceId, isPaymentDone, isLocked, email, socket,lockedStatus }) =>
  async (dispatch) => {
    try {
      dispatch({
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/devices/payment-update-for-email?DeviceId=${DeviceId}&isPaymentDone=${isPaymentDone}&isLocked=${isLocked}&lockedStatus=${lockedStatus}`,
        config
      );
      ...
      if (data.statusCode === 200) {
        alert(data?.message);
        // ✅ Emit event using passed socket
        socket?.emit("ReactRequestForPaymentStatus", DeviceId);
        setTimeout(() => {
          window.location.reload();
        }, 500);
      }
      ...
      dispatch({
        ...
        payload:
          error?.response?.data?.data?.err?.msg || "Something went wrong",
      });
    }
  };
