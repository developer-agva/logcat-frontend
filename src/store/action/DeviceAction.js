...
export const getUserDeviceAction = ({ page, limit, searchData }) => async (dispatch) => {
  try {
    dispatch({
    ...
    let response;
    response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/api/logger/logs/AllEvents/get-devices-for-users?search=${searchData}&page=${page}&limit=${limit}`,
      config
    );
    dispatch({
...
export const getSingleProductionDataAction = (serialNumber) => async (dispatch) => {
  try {
    dispatch({
