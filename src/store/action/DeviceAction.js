...
export const getDeviceEventsById =
  ...
  async (dispatch) => {
    try {
      ...
      response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/logger/logs/deviceEvents/${DeviceId12}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`,
        config
      );
      dispatch({
...
export const getDeviceAlarmsById =
  ...
  async (dispatch) => {
    try {
      ...
      response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/logger/logs/deviceAlerts/${DeviceId121}?page=${page}&limit=${limit}&startDate=${startDatee}&endDate=${endDatee}`,
        config
      );
      dispatch({
