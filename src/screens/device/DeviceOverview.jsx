...
function DeviceOverview() {
  ...
  const handleLive = (e) => {
    e.preventDefault()
    navigate(`/live?code=${code}&projectName=${projectName}&DeviceId=${deviceid}`)
  }
  ...
  const handleBackBtn = () => {
    if (adminProfile == "User") {
      ...
      history(
        `/salesSideBar?name=AgVa Pro ATP&projectCode=008`
      );
    }
    else {
  ...
  const handelLock = () => {
    let email = "ceo@agvahealthtech.com"
    if (isPaymentComplete === "true") {
      dispatch(
        postLockToDeviceIdAction({ DeviceId: deviceid, isPaymentDone: "false", isLocked: false, email })
      );
    } else {
      dispatch(
        postLockToDeviceIdAction({ DeviceId: deviceid, isPaymentDone: "true", isLocked: false, email })
      );
    }
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            {code === "003" || code === "004" || code === "007" || code === "008" ? (
              <Link
              ...
              <Link
                to={`/deviceEvents?code=${code}&projectName=${projectName}&DeviceId=${deviceid}`}
                style={{ textDecoration: "none" }}
                onClick={() => localStorage.setItem("deviceid", deviceid)}
              >
                <div
            ...
            {adminProfile == "Super-Admin" || adminProfile == "Marketing-Admin" ? (
              <Link
