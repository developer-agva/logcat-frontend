...
function DeviceOverview() {
  ...
  const handleBackBtn = () => {
    ...
    else if(projectCode==='004' ||projectCode==='003'){
      history(`/newDevice?code=${code}&name=${projectName}&projectCode=${projectCode}`)
    }
    else {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ position: "absolute", top: "5rem", left: "1rem", width: '100vw' }}
      >
        <Toaster />
        <div
        ...
        >
          ...
          <div>
            ...
            >
              <Card
              >
                <form className='flex justify-between flex-wrap' style={{ gap: '2rem' }}>
                  <div className="flex flex-col gap-4">
                    <section style={{ width: 'full', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ width: '10rem' }}>Device ID :</span>
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{overviewData && overviewData.DeviceId}</h5>
                    </section>
                    <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ width: '10rem' }}>Running Status :</span>
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{status === "ACTIVE" ? <>
                    ...
                    <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{status == "ACTIVE" ? "In Ventilation" : lastHours}</h5>
                    </section>
                  </div>
                  <div className="flex flex-col gap-4">
                    <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ width: '11rem' }}>Total Ventilatin Hours :</span>
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{!totalHours ? "- - -" : totalHours}</h5>
                    </section>
                    <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ width: '11rem' }}>Health :</span>
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{!health ? "- - -" : health}</h5>
                    </section>
                    <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ width: '11rem' }}>Address :</span>
                      <h5 style={{ width: '9rem', fontSize: '0.9rem' }} >{!address ? "- - -" : address}</h5>
                    </section>
          ...
          <div className="container" style={{ borderRadius: "15px", padding: "0.1rem", marginLeft: "0px", color: "#707070" }}>
            ...
            <div className="d-flex" style={{ gap: "2rem", textAlign: "center", flexWrap: 'wrap' }}>
              ...
              <Link to={`/newDeviceEvents?code=${code}&projectName=${projectName}&DeviceId=${deviceid}&projectCode=${projectCode}`} style={{ textDecoration: "none" }} onClick={() => localStorage.setItem("deviceid", deviceid)}>
            ...
            :
              <Link to={`/deviceEvents?code=${code}&projectName=${projectName}&DeviceId=${deviceid}&projectCode=${projectCode}`} style={{ textDecoration: "none" }} onClick={() => localStorage.setItem("deviceid", deviceid)}>
                <div style={{ alignItems: 'center', justifyContent: "space-around", background: "#FFFFFF 0% 0% no-repeat padding-box", boxShadow: "0px 0px 50px #00000029", display: "flex", padding: "15px", width: "11rem", borderRadius: "10px", color: "#707070" }}>
                  <h6>Monitor Data</h6>
                  <span style={{ width: '0.8rem', height: '0.8rem', backgroundColor: 'red', borderRadius: '10px' }}></span>
                </div>
              ...
              {adminProfile == 'Doctor' ?
                <Link to={`/deviceAssignToUserModel?code=${code}&projectName=${projectName}&DeviceId=${deviceid}`} style={{ textDecoration: "none" }}>
                  <div style={{ justifyContent: "space-around", background: "#FFFFFF 0% 0% no-repeat padding-box", boxShadow: "0px 0px 50px #00000029", display: "flex", alignItems: 'center', padding: "15px", width: "11rem", borderRadius: "10px", color: "#707070", textAlign: 'center' }}>
                    <h6>Assign</h6>
                  </div>
                </Link>
                : ''}
              ...
              {adminProfile == 'Hospital-Admin' ?
                <Link to={`/hospitalAdmoinRemoveDoctor?code=${code}&projectName=${projectName}&DeviceId=${deviceid}`} style={{ textDecoration: "none" }}>
                  <div style={{ justifyContent: "space-around", background: "#FFFFFF 0% 0% no-repeat padding-box", boxShadow: "0px 0px 50px #00000029", display: "flex", alignItems: 'center', padding: "15px", width: "11rem", borderRadius: "10px", color: "#707070", textAlign: 'center' }}>
                    <h6>Assiged Owner</h6>
                  </div>
                </Link>
