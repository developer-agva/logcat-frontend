...
import {
  ...
  getPatientDetailsOfNewDevicesByUhid,
} from "../../../../store/action/DeviceAction";
import { useNavigate, useNavigation } from "react-router";
import { Button, Modal } from "flowbite-react";
export default function PatientDetailsOfNewData() {
  ...
  const getPatientNewDetailsOfDevicesByUhidReducer = useSelector(
    (state) => state.getPatientNewDetailsOfDevicesByUhidReducer
  );
  const { data: dataa } = getPatientNewDetailsOfDevicesByUhidReducer;
  console.log("singleData", dataa);

  useEffect(() => {
    dispatch(getPatientDetailsOfNewDevicesByUhid(deviceid));
  }, []);

  const [openModal, setOpenModal] = useState();
  const [detailsData, setDetailsData] = useState();
  const props = { openModal, setOpenModal };
  let calibrationFilter = dataa;
  const incPage = parseInt(dataa && dataa.currentPage);
  const totalPage = parseInt(dataa && dataa.totalPages);
  ...
  const records =
    calibrationFilter && calibrationFilter.slice(firstIndex, lastIndex);
  const npage = Math.ceil(dataa && dataa.length / recordsPerPage);
          ...
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                <td
                  scope="col"
                  class="px-6 py-3 text-center text-black text-4xl font-semibold"
                >
                  UHID
                </td>
                <td
                    ...
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        >
                          ...
                          <Modal.Body>
                            <div className="space-y-6">
                              ...
                              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Patient Name:{" "}
                                {dataa && dataa.patientName
                                  ? dataa.patientName
                                  : "- - -"}
                              </p>
                              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Patient Age:{" "}
                                {dataa && dataa.age ? dataa.age : "- - -"}
                              </p>
                              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Patient Height:{" "}
                                {dataa && dataa.height ? dataa.height : "- - -"}
                              </p>
                              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Patient Weight:{" "}
                                {dataa && dataa.weight ? dataa.weight : "- - -"}
                              </p>
                              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                Dosage Provided:{" "}
                                {dataa && dataa.dosageProvided
                                  ? dataa.dosageProvided
                                  : "- - -"}
                              </p>
                            </div>
                          </Modal.Body>
                      ...
                      <td class="px-6 py-4 text-center ">
                        <button
                          onClick={() =>
                            navigate(
                              `/patientDetails?uhId=${item1.UHID}&deviceId=${item1.deviceId}`
                            )
                          }
