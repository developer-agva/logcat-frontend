...
export default function Events() {
  ...
  const { loading, data, error } = getAllEventsByDeviceIdReducer;
  const eventsFilter = data?.data?.findDeviceById ? data?.data?.findDeviceById : data?.data; // Safeguard added
  console.log('data', data,eventsFilter)

  const incPage = parseInt(data && data.currentPage) || 1; // Default fallback to 1
  const totalPage = parseInt(data && data.totalPages) || 1; // Default fallback to 1
  ...
  useEffect(() => {
    dispatch(getDeviceEventsById({ page: 1, limit: recordsPerPage, search: '' }));
  }, [])
  return (
    <>
