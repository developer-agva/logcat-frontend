...
function AddSalesDetails() {
    ...
    const handleSubmit = useCallback((e) => {
        e.preventDefault();
        const { deviceId, serialNumber, contactNo,amount, hospitalName, demoDuration, priority, description } = demoData;
        ...
        dispatch(postDemoAddDataAction({ deviceId, serialNumber, contactNo, amount, hospitalName, demoDuration, priority, description }));
    }, [demoData, dispatch]);
    return (
        <div>
            <Toaster />
            <div style={{ marginTop: '6rem' }}>
                <form className="px-4 flex flex-col gap-4" onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-col gap-2">
                        <label htmlFor="amount" className="text-sm md:text-base">Amount in (INR)</label>
                        <input
                            id="amount"
