...
import { getDeviceAlarmsById, getDeviceEventsById } from '../../store/action/DeviceAction';
export default function DeviceLogs() {
  ...
  const handleChange = (start) => {
    setStartValue(start?.join(","));
    console.log('Start Date:', start?.join(",")?.split(",")[0]?.slice(0, 4));
    const startDate = start?.join(",")?.split(",")[0]
    const endDate = start?.join(",")?.split(",")[1]

    // End Date Formate Code
    const date1 = new Date(endDate);
    const year1 = date1.getFullYear();
    const month1 = String(date1.getMonth() + 1).padStart(2, '0'); // Months are zero-based
    const day1 = String(date1.getDate()).padStart(2, '0');
    const endDateFormate = `${year1}-${month1}-${day1}`;

    // Start Date Formate Code
    const date = new Date(startDate);
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0'); // Months are zero-based
    const day = String(date.getDate()).padStart(2, '0');
    const startDateFormate = `${year}-${month}-${day}`;

    if (activeTab == 0) {
     return dispatch(getDeviceEventsById({ page: 1, limit: '', startDate: startDateFormate,endDate:endDateFormate }))
    }
    else if(activeTab == 1 ) {
     return dispatch(getDeviceAlarmsById({ page: 1, limit: '',startDate:startDateFormate, endDate: endDateFormate }))
    }
    else{
      return;
    }
                    ...
                    <>
                      <div className={`${Style.Tabs} `}>
                        ...
                        {activeTab == 0 || activeTab == 1 ? <>
                          <input type="text" placeholder='Search...' onChange={(e) => { console.log('searchData', e.tagte.value) }} style={{ padding: '10px', width: '40%' }} />
                          <DateRangePicker
