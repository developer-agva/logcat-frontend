...
import { getUserDeviceAction } from '../../../store/action/DeviceAction';
...
function ProductionDataModule() {
    ...
    const handleClickSearch = () => {
        if (query && query.length > 0) {
            dispatch(getproductionDetailsAction({ page: 1, limit: recordsPerPage, searchData: query }));
        }
        else {
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <h1 class="text-2xl font-extrabold">Master<small class="ml-2 font-semibold text-gray-500 dark:text-gray-400">Data</small></h1>
                            </div>
                        </div>
                        {/* Events  */}
                                            ...
                                            {records && records.map((item1, index) => {
                                                return (
                                                    <tr class="bg-white border-b hover:bg-gray-50">
                                                        ...
                                                        <td class="px-6 py-4 text-left">
                                                            {item1.manufacturingDate ? item1.manufacturingDate : 'NA'}
                                                        </td>
                                                        <td class="px-6 py-4 text-left">
                                                            <button onClick={() => {
                                        ...
                                        <div className="text-center">
                                            ...
                                            <div className="flex justify-center gap-4">
                                                {dhrFileLink && dhrFileLink.length > 0 ?
                                                    <Link to={dhrFileLink} rel='prerender' style={{ textDecoration: 'none', backgroundColor: 'red', color: 'white', padding: '10px', borderRadius: '8px' }}>
                                                        View Master File
                                                    </Link>
                                                    :
                                                    <Link style={{ textDecoration: 'none', backgroundColor: 'red', color: 'white', padding: '10px', borderRadius: '8px' }}>
                                                        No Master File
                                                    </Link>
                                                }
                                                <button onClick={() => { history(`/reportDataForProduction?deviceId=${deviceIdForReport}`) }} style={{ backgroundColor: 'white', border: '0.5px solid gray', color: 'black', padding: '10px', borderRadius: '8px' }}>
                                                    View Service Report
                                                </button>
                                                <button onClick={() => { history(`/singleProductionDataWithEdit?serialNo=${serialNoForSingle}&deviceId=${deviceIdForReport}`) }} style={{ backgroundColor: 'white', border: '0.5px solid gray', color: 'black', padding: '10px', borderRadius: '8px' }}>
                                                 View Data
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <ul class="pagination justify-content-end" style={{ display: "flex", alignItems: 'center' }}>
                                        ...
                                        {incPage !== totalPage ?
                                            <button onClick={nextPage} style={{ border: "0px", backgroundColor: "white" }}>
                                                <img src={back} loading='lazy' style={{ width: "3rem", transform: "rotate(180deg)" }} />
                                            </button>
                                            : " "}
                                    </ul>
                                </nav>
