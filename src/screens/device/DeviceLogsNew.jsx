...
export default function DeviceLogsNew() {
  ...
  const handleTabClick = (tabIndex) => {
    ...
    if (tabIndex == 5) {
      setPatientDischarge('btn text-black')
    }
    else {
      setPatientDischarge('btn-light')
    }
    if (tabIndex == 4) {
                      ...
                      <div className={`${Style.Tabs} `}>
                        <div>
                          ...
                          {adminProfile == 'Super-Admin' || adminProfile == 'Nurse' ?
                            <button className={patientDetailsbtn} onClick={() => handleTabClick(4)} style={{ width: "10rem", padding: "6px", border: 'none', color: "#fff", fontSize: '0.9rem' }}>Patient Details</button>
                            : ''}
                          <button className={patientDischarge} onClick={() => handleTabClick(5)} style={{ width: "9rem", padding: "6px", border: 'none', color: "#fff", fontSize: '0.9rem' }}>Patient Discharge</button>
                        </div>
                      </div>
                      <section className={`${Style.tableHeader}`}>
                        {activeTab === 0 && <EventsOfNewDevices />}
                        {activeTab === 1 && <AlarmsOfNewDevices />}
                        {activeTab === 2 && <LogsOfNewDevices />}
                        {projectCode === "004" ?
                          activeTab === 3 && <TrendsOfNewData /> : activeTab === 3 && <Trends />}
                        {activeTab === 4 && <PatientDetailsOfNewData />}
                        {activeTab === 5 && <PatientDischarge />}
                      </section>
                    </>
                  </TableCard1>
                </div>
