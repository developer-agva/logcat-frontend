...
import { appDisapprovePendingDataAction, getInactiveUserAction } from '../../../store/action/AdminDashboard';
...
function InActiveUsers() {
  ...
  const getInacticeUserReducer = useSelector(
    (state) => state.getInacticeUserReducer
  );
  const { loading, data, error } = getInacticeUserReducer;
  const registerUsers = data && data.data;
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getInactiveUserAction({ page: 1, limit: 10 }));
  }, []);
                    ...
                    return (
                      <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-center ">
                          <button onClick={() => {
                            setOpenModal(true)
                            setInactive(item._id)
                          }} style={{ backgroundColor: 'rgb(152, 0, 76)', color: 'white', padding: '10px', borderRadius: '5px' }}>Active</button>
                        </td>
                ...
                <div className="text-center">
                  ...
                  <div className="flex justify-center gap-4">
                    <button style={{ backgroundColor: 'red', color: 'white', padding: '10px', borderRadius: '8px' }} onClick={() => {
                      if(inactive){
                        dispatch(appDisapprovePendingDataAction({ accountStatus: 'Active', userId: inactive }))
                        setTimeout(() => {
                          setOpenModal(false)
                        }, 200);
