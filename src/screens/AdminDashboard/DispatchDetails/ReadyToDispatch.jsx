...
import { getReadyToDispatchDetailsAction } from '../../../store/action/DispatchDetailsAction';
import { useState } from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
function ReadyToDispatch() {
    const getReadyForDispatchDataReducer = useSelector((state) => state.getReadyForDispatchDataReducer);
    const { loading, data } = getReadyForDispatchDataReducer;
    const getDispatchData = data && data.data
    const dispatch = useDispatch()
    useEffect(() => {
        dispatch(
            getReadyToDispatchDetailsAction({ page: 1, limit: recordsPerPage })
        )
    }, ([]))
    ...
    const handleClickSearch = () => {
        if (query && query.length > 0) {
            dispatch(getReadyToDispatchDetailsAction({ page: 1, limit: recordsPerPage, searchData: query }));
        }
        else {
                                                    ...
                                                    <tr class="bg-white border-b hover:bg-gray-50">
                                                        ...
                                                        <td class="px-6 py-4 text-left">
                                                            {item1.shipmentMode === 'req_doc' ?
                                                                <button onClick={() => {
                                                                    history(`/dispatchDevice?deviceId=${item1.deviceId}&serialNum=${item1.serialNumber}&product=${item1.productType}&purpose=${item1.purpose}`)
                                                                ...
                                                                item1.shipmentMode === 'inprocess' ?
                                                                    <button style={{ textDecoration: 'none', backgroundColor: 'rgb(203, 41, 123)', color: 'white', padding: '10px', width: '7vw', borderRadius: '5px' }}>
                                                                        In Process
                                                                    </button>
                                                                    :
                                                                    <button style={{ textDecoration: 'none', backgroundColor: 'rgb(203, 41, 123)', color: 'white', padding: '10px', width: '7vw', borderRadius: '5px' }}>
    ...
    function prePage() {
        dispatch(getReadyToDispatchDetailsAction({ page: incPage - 1, limit: recordsPerPage }))
    }
