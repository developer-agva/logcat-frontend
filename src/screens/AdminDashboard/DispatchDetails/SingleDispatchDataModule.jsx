...
import { getSingleSerialNumberData } from '../../../store/action/DispatchDetailsAction';
;

function SingleDispatchDataModule() {
    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    const deviceId = urlParams.get('deviceId')

    // Single Serial No Tracker Data
    const getSingleSerialNoTrackReducer = useSelector((state) => state.getSingleSerialNoTrackReducer);
    const { data } = getSingleSerialNoTrackReducer;
    const singleData = data && data.data

    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(getSingleSerialNumberData(deviceId))
    }, [])
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <h1 class="text-2xl font-extrabold">Dispatch Single<small class="ml-2 font-semibold text-gray-500 dark:text-gray-400">Data</small></h1>
                                    ...
                                    {singleData && singleData.map((item1, index) => {
                                        return (
                                            <tr class="bg-white border-b hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 w-25 text-center">
                                                    {item1.address}
                                                </td>
                                            </tr>
                                        )
...
export default SingleDispatchDataModule