...
import { dispatchDetailsAction, getDeviceIdFromProduction, getPincodeData, getSingleHospitalDetails, getproductionDetailsByIdAction } from '../../../store/action/DispatchDetailsAction'
...
function Dispatch() {
    ...
    const hospitalCountry = singleDataOfHospital && singleDataOfHospital[0] && singleDataOfHospital[0].Country
    ...
    const districtNewData = dispatchDetails.nDistrict.length > 0 ? dispatchDetails.nDistrict : hospitalDistrict
    const cityNewData = dispatchDetails.nCity.length > 0 ? dispatchDetails.nCity : hospitalCity
    const stateNewData = dispatchDetails.nState.length > 0 ? dispatchDetails.nState : hospitalState
    const countryNewData = dispatchDetails.nCountry.length > 0 ? dispatchDetails.nCountry : hospitalCountry

    const [selectedImage, setSelectedImage] = useState(null);
    const [loadinState, setLoadingState] = useState(false)
    const [poFileSelect, setPoFileSelect] = useState(false)
    const [pdfUrl, setPdfUrl] = useState('');
    const [getState, setGstState] = useState();
    const [getStateData, setGstStateData] = useState();
    ...
    const handleDateChange = (event) => {
        ...
        if (dateValue) { 
          ...
          setDispatchDetails({ ...dispatchDetails, date_of_dispatch: format(parsedDate, 'd MMMM yyyy') })
        } else {
          setDispatchDetails('');
        }
      };
    ...
    const verifyGst = async (e) => {
        e.preventDefault();
        const gstNo = dispatchDetails && dispatchDetails.distributer_gst;
        try {
            setLoadingState(true);
            const response = await axios.get(`http://sheet.gstincheck.co.in/check/dbec163f440a30b179fd388282bf06ae/${gstNo}`,
                {
                    headers: { 'Content-Type': 'multipart/form-data' },
                });
            setGstStateData(response && response.data && response.data.data)
            setGstState(response && response.data && response.data.flag);
        } catch (error) {
            console.error('Error generating PDF:', error);
        }
    }
                ...
                <div className={Style.dispatchContainer}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "1rem", }}>
                            <button onClick={()=> navigate(`/dispatchDashboardModule?activeTab=${activeTab}`)} style={{ display: 'block' }}>
                        ...
                        >
                            ...
                            >
                                <form className='w-full flex gap-12 flex-wrap justify-between'>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        <section style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <span style={{ color: 'black', width: '5rem', textAlign: 'start' }}>SO File:</span>
                                ...
                                <form className='w-full flex justify-between flex-wrap'>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            ...
                                            <input type="text" onChange={(e) => setDispatchDetails({ ...dispatchDetails, distributor_contact: e.target.value })}
                                        ...
                                        <section style={{ display: 'flex', alignItems: 'center', }}>
                                            ...
                                            {dispatchDetails.distributer_gst.length > 0 ?
                                                <button onClick={verifyGst} style={{ padding: '10px', backgroundColor: 'black', borderRadius: '8px', color: 'white' }}>
                                                    Ckeck
                                                </button>
                                                : ''}
                                        </section>
                                        <section style={{ display: 'flex', alignItems: 'center', }}>
                                            {dispatchDetails.distributer_gst.length > 0 ?
                                                getState && getState === true ?
                                                    <div style={{ display: 'flex', alignItems: 'start', gap: '10px', flexDirection: 'column' }}>
                                                        <div>
                                                            <span style={{ color: 'black' }}>Trade Name: </span>
                                                            <span>{getStateData && getStateData.tradeNam}</span>
                                                        </div>
                                                        <div>
                                                            <span style={{ color: 'black' }}>Address: </span>
                                                            <span>{getStateData && getStateData.pradr && getStateData.pradr.adr.split(',')[0]} , {getStateData && getStateData.pradr && getStateData.pradr.adr.split(',')[1]} ,{getStateData && getStateData.pradr && getStateData.pradr.adr.split(',')[3]}, {getStateData && getStateData.pradr && getStateData.pradr.adr.split(',')[6]}</span>
                                                        </div>
                                                    </div> :
                                                    getState && getState === false ?
                                                        <div style={{ display: 'flex', alignItems: 'start', gap: '10px', flexDirection: 'column' }}>
                                                            <span style={{ color: 'black' }}>Invalid GSTIN Number</span>
                                                        </div>
                                                        : ''
                                                : ''}
                                        </section>
                        ...
                        >
                            ...
                            >
                                <form className='w-full flex justify-between flex-wrap'>
                                    ...
                                    value={selectedDate}
                                             style={{ width: '20rem' }}
                                                id="visitors" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-700 :border-gray-600" required />
                                        </section>
                                        {dispatchDetails && dispatchDetails.hospitalName && dispatchDetails.hospitalName.length > 0 ?
                                            <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <span style={{ color: 'black', width: '12rem', textAlign: 'start' }}>City :</span>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        {dispatchDetails && dispatchDetails.hospitalName && dispatchDetails.hospitalName.length > 0 ?
                                            <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <span style={{ color: 'black', width: '7rem', textAlign: 'start' }}>Country :</span>
                                                <div style={(singleDataOfHospital && singleDataOfHospital[0] && singleDataOfHospital[0].Pincode && singleDataOfHospital[0].Hospital_Address.length > 0) ? { width: '20rem', padding: '0.6rem', textAlign: 'start' } : { width: '20rem', padding: '1.2rem', textAlign: 'start' }} id="company" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-700 :border-gray-600">
                                                    {hospitalCountry}
                                                </div>
                                            </section>
                                            ...
                                            <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <span style={{ color: 'black', width: '7rem', textAlign: 'start' }}>Country :</span>
                                                <input type="text" onChange={(e) => setDispatchDetails({ ...dispatchDetails, nCountry: e.target.value })} style={{ width: '20rem' }}
                                                    value={dispatchDetails.nCountry} id="visitors" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-700 :border-gray-600" placeholder='Enter Country' required />
                                            </section>}
                                    </div>
                                </form>
                            </Card >
