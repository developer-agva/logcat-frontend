...
import { getReturnDataRequestByDispatchAction, getaccountsPendingDetailsAction } from '../../../store/action/DispatchDetailsAction';
import { useState } from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
function ReturnRequests() {

    const getDispatchPendingDataReducer = useSelector((state) => state.getDispatchPendingDataReducer);
    const { loading, data } = getDispatchPendingDataReducer;
    const getDispatchData = data && data.data
    console.log('getDispatchData', getDispatchData)
    const dispatch = useDispatch()
    useEffect(() => {
        dispatch(
            getReturnDataRequestByDispatchAction({ page: 1, limit: recordsPerPage })
        )
    }, ([]))
    ...
    const handleClickSearch = () => {
        if (query && query.length > 0) {
            dispatch(getReturnDataRequestByDispatchAction({ page: 1, limit: recordsPerPage, searchData: query }));
        }
        else {
                                        ...
                                        {records && records.map((item1, index) => {
                                            return (
                                                <tr class="bg-white border-b hover:bg-gray-50">
                                                    ...
                                                    <td class="px-6 py-4 text-left">
                                                        {item1.concerned_person_email ? item1.concerned_person_email : 'NA'}
                                                    </td>
                                                    <td class="px-6 py-4 text-left">
                                                    ...
                                                    <td class="px-6 py-4 text-left">
                                                        {item1.shipmentMode === 'req_doc' ?
                                                            <button onClick={() => {
                                                                history(`/dispatchDevice?deviceId=${item1.deviceId}&serialNum=${item1.serialNumber}`)
                                                            }} style={{ textDecoration: 'none', backgroundColor: 'rgb(203, 41, 123)', color: 'white', padding: '10px', width: '7vw', borderRadius: '5px' }}>
                                                                Request Doc.
                                                            </button>
                                                            :
                                                            item1.shipmentMode === 'inprocess' ?
                                                                <button onClick={() => { history(`/accountFormModule?serialNo=${item1.serialNumber}`) }} style={{ textDecoration: 'none', backgroundColor: 'rgb(203, 41, 123)', color: 'white', padding: '10px', width: '7vw', borderRadius: '5px' }}>
                                                                    Upload Data
                                                                </button>
                                                                :
                                                                <button onClick={() => { history(`/accountFormModule?serialNo=${item1.serialNumber}`) }} style={{ textDecoration: 'none', backgroundColor: 'rgb(203, 41, 123)', color: 'white', padding: '10px', width: '7vw', borderRadius: '5px' }}>
    ...
    function prePage() {
        dispatch(getaccountsPendingDetailsAction({ page: incPage - 1, limit: recordsPerPage }))
    }
