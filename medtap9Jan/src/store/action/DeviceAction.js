...
export const trackHistorydataAction =
  ({ deviceId, state, city, ASM_RSM, hospitalName, purpose, serialNumber,startDate,endDate }) =>
  async (dispatch) => {
    try {
      dispatch({
...
export const getAllDevicesDataAgvaMiniAction =
  ({ page = 1, limit = 10, searchData = "", filter } = {}) =>
  async (dispatch) => {
    try {
      dispatch({
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/marketing/sales/get-all-device-track-history/007?page=${page}&limit=${limit}&search=${searchData}&filter=${filter}`,
        config
      );
