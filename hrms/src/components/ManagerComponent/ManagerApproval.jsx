  ...
  const renderTableRows = (data, isCompOff = false) =>
    data.map((item, index) => (
      <tr key={index} className="border-t">
        ...
        <td className="p-5 text-center" style={{width:'20rem'}}>{item?.reason}</td>
        <td className="p-5 text-center">{item?.totalDays}</td>
        <td className="p-5 text-center">
        {item?.location?
          <Link loading='lazy' className="py-2 px-3 bg-blue-500 text-white rounded" to={item?.location}>View</Link>
          :'---'
        }
        </td>
